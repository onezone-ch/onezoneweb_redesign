...
| [VehicleSearchModal.tsx](../src/features/automation-form/VehicleSearchModal.tsx) | Ricerca veicolo: matricola (9 cifre auto-search) / n. omologazione (≥6 char) / marca+filtri (brand autocomplete → modelli da `action=filters`, potenza min/max); tabella sticky-header con paginazione; selezione → compila brand/model/n_certificate(/serial se matricola) |
